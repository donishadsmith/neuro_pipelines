...
homepage = st.Page(
    "_homepage.py", title="Home"
)
bids_conversion = st.Page(
    "bids_conversion_pipeline/streamlit/bids_conversion_app.py", title="NIfTI to BIDS Conversion Pipeline"
)
...
pg = st.navigation([homepage, bids_conversion, participants_tsv, add_dosages, move_files, events, connors], position="top")
pg.run()
