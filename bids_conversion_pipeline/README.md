...
When sessions are missing, a subjects visits file ensures that dates are mapped to the correct session IDs. The file must contain `participant_id` and `date` columns. List all visit dates in chronological order per subject and use `NaN` for missing sessions:
...
python cli/bids_conversion_cli.py \
    ...
    --bids_dir /path/to/bids/dir \
    --subjects_visits_file /path/to/visits.csv \
    --subjects_visits_date_fmt %m/%d/%Y \
    --src_data_date_fmt %y%m%d \
    --subjects 100 101
```
